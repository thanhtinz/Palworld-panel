...
from .domain import build_dashboard_state, build_servers, build_event_topics, to_jsonable
...
class PalcoreRequestHandler(BaseHTTPRequestHandler):
    """Small stdlib HTTP handler for the bundled panel and API."""
    ...
    def do_GET(self) -> None:  # noqa: N802 - stdlib handler naming
        ...
        if path == "/":
            ...
            self._send_json(to_jsonable(build_servers()))
        elif path == "/api/v1/pals/system":
            state = build_dashboard_state()
            self._send_json({"pals": to_jsonable(state.pals), "pal_system": to_jsonable(state.pal_system)})
